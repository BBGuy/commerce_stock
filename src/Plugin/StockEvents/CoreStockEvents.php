...
class CoreStockEvents extends PluginBase implements StockEventsInterface {
  ...
  public function stockEvent($context, PurchasableEntityInterface $entity,
    ...
    $order_update_events = [StockEventsInterface::ORDER_UPDATE_EVENT, StockEventsInterface::ORDER_ITEM_UPDATE_EVENT];
    $order_delete_events = [StockEventsInterface::ORDER_CANCEL_EVENT, StockEventsInterface::ORDER_DELET_EVENT, StockEventsInterface::ORDER_ITEM_DELETE_EVENT];
    ...
    // Get the stock serive
    $stockService = \Drupal::service('commerce_stock.service_manager')->getService($entity);
    // Use the stock updater to create the transaction.
    $transaction_id = $stockService->getStockUpdater()->createTransaction($entity, $location->getId(), '', $quantity, NULL, $transaction_type, $metadata);
