...
class OrderEventSubscriber implements EventSubscriberInterface {
    ...
    foreach ($order->getItems() as $item) {
      if (!$original_order->hasItem($item)) {
        if ($order && !in_array($order->getState()->value, ['draft', 'canceled'])) {
          ...
          $service->getStockUpdater()->createTransaction($entity->id(), $location, '', $amount, NULL, StockTransactionsInterface::STOCK_SALE, $metadata);
        }
      }
    }
  }
  ...
  public function onOrderCancel(WorkflowTransitionEvent $event) {
    ...
    foreach ($order->getItems() as $item) {
      ...
      if ($checker->getIsStockManaged($entity)) {
        ...
        $service->getStockUpdater()->createTransaction($entity->id(), $location, '', $quantity, NULL, StockTransactionsInterface::STOCK_RETURN, $metadata);
      }
    }
  }
  ...
  public function onOrderItemDelete(OrderItemEvent $event) {
    ...
    if ($order && !in_array($order->getState()->value, ['draft', 'canceled'])) {
      ...
      $service->getStockUpdater()->createTransaction($entity->id(), $location, '', $item->getQuantity(), NULL, StockTransactionsInterface::STOCK_RETURN, $metadata);
    }
  }
